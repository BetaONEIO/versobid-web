...
// Response type from bidService.createBid
...
export const BidForm: React.FC<BidFormProps> = ({ item, onBidSubmitted }) => {
  ...
  const handleChange = (field: keyof BidFormData, value: string | number) => {
    setFormData((prev: BidFormData) => ({ ...prev, [field]: value })); // Explicitly typed 'prev'
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response: CreateBidResponse = await bidService.createBid({
        itemId: item.id,
